...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (!pDevice || !pContext) {
		hal::dout << "Resource/Shader/Shader_Initialize() : 与えられたデバイスかコンテキストが不正です" << std::endl;
		return false;
	}
	...
	//　サンプラーステート設定
	D3D11_SAMPLER_DESC sampler_desk{};

	//　フィルタリング
	sampler_desk.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;

	//　uv参照外の取り扱い
	sampler_desk.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
	sampler_desk.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
	sampler_desk.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
	sampler_desk.MipLODBias = 0;
	sampler_desk.MaxAnisotropy = 8;
	sampler_desk.ComparisonFunc = D3D11_COMPARISON_ALWAYS;
	sampler_desk.MinLOD = 0;
	sampler_desk.MaxLOD = D3D11_FLOAT32_MAX;

	g_pDevice->CreateSamplerState(&sampler_desk, &g_pSamplerState);
